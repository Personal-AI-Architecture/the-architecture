...
**Related documents:** [foundation-spec.md](./foundation-spec.md) (architecture overview, links to all component specs), [memory-tool-completeness.md](./memory-tool-completeness.md) (completeness proof — why the architecture needs no new components)
...
Both exceptions trace to the same root: making intelligence swappable. The Model needs its own API so you *can* swap it. Auth needs independence so swapping it doesn't *break* security. One cause, two consequences. See [memory-tool-completeness.md](./memory-tool-completeness.md) §2 for the full argument.
