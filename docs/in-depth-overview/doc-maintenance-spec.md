...
```json
{
  "docs": [
    {
      "path": "docs/in-depth-overview/engine-spec.md",
      "covers": ["src/engine/"],
      "tier": 1,
      "update_trigger": "code",
      "depends_on": ["docs/in-depth-overview/foundation-spec.md"]
    }
  ]
}
```
...
check:docs — documentation freshness check
  ...
  STALE  docs/in-depth-overview/engine-spec.md
         covers: src/engine/
         changed: src/engine/agent-loop.ts (+42 -18)
         action: Tier 2 — update required before merge

  STALE  AGENT.md
         depends_on: docs/in-depth-overview/foundation-spec.md (modified)
         action: Tier 3 — AI agent will auto-update

  OK     docs/in-depth-overview/memory-spec.md
  OK     docs/in-depth-overview/auth-spec.md
  ...
      ...
      - uses: anthropics/claude-code-action@v1
        with:
          ...
          prompt: |
            ...
            Read docs/in-depth-overview/doc-maintenance-spec.md for the full maintenance rules and constraints.
...
The lock-in gate (`docs/in-depth-overview/lockin-gate.md`) gets one new check:
...
2. A PR that modifies `src/engine/` without modifying `docs/in-depth-overview/engine-spec.md` causes `check:docs` to exit 2
